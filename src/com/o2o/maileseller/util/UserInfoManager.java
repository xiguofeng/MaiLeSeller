...
public class UserInfoManager {
	...
	public static void saveUserInfo(Context context, String userName,
		...
		if (null != user) {
			...
			userInfo.putString(USER_ID_KEY,
					null == user.getId() ? "" : user.getId());

			userInfo.putString(USER_NAME_KEY, userName);
			userInfo.putString(USER_PWD_KEY, userPass);
			userInfo.putString(USER_REAL_NAME_KEY, user.getRealName());
			userInfo.putString(USER_COMPANY_NAME_KEY, user.getCompanyname());
			userInfo.putString(USER_PROVINCE_KEY,
					null == user.getProvince() ? "" : user.getProvince());
			userInfo.putString(USER_CITY_KEY, null == user.getCity() ? ""
					: user.getCity());

			userInfo.putString(USER_SEX_KEY,
					null == user.getSex() ? "" : user.getSex());
			userInfo.putString(USER_SIGNATURE_KEY,
					null == user.getSignature() ? "" : user.getSignature());
			userInfo.putString(USER_BIRTHDAY_KEY,
					null == user.getBirthday() ? "" : user.getBirthday());
			userInfo.putString(USER_LAST_LOGINTIME_KEY, null == user
	...
	public static void clearUserInfo(Context context) {
		...
		userInfo.putString(USER_NAME_KEY, "");
		userInfo.putString(USER_PWD_KEY, "");
		userInfo.putString(USER_REAL_NAME_KEY, "");
		userInfo.putString(USER_COMPANY_NAME_KEY, "");
		userInfo.putString(USER_PROVINCE_KEY, "");
		userInfo.putString(USER_CITY_KEY, "");

		userInfo.putString(USER_BIRTHDAY_KEY, "");
		userInfo.putString(USER_SEX_KEY, "");
		userInfo.putString(USER_SIGNATURE_KEY, "");
		userInfo.putString(USER_LAST_LOGINTIME_KEY, "");

		userInfo.putBoolean(USER_LOGIN_IN_IS_AUTO, false);
		userInfo.commit();
	...
	public static void setRememberPwd(Context context, Boolean isRemember) {
		...
		userInfo.putBoolean(USER_REMEMBER_PSW, isRemember);
		userInfo.commit();
